...
export function classifyTask(text: string): TaskCategory {
  ...
  if (/\b(research|cite|source|paper|study|analyze market)\b/.test(t))
    return "research";
  if (/\b(prove|math|logic|reason|calculate)\b/.test(t)) return "reasoning";
  if (/\b(story|poem|creative|marketing|campaign)\b/.test(t)) return "creative";
  if (/\b(image|video|photo)\b/.test(t)) return "multimodal";
  if (/\b(trend|news|twitter|x\.com)\b/.test(t)) return "realtime";
  return "general";
}
